	...
	return [
		{
			...
			filterFn: (row, columnId, filterValue) => {
				...
				return (
					(network.filename?.toLowerCase().includes(searchStr) ?? false) ||
					(network.name?.toLowerCase().includes(searchStr) ?? false) ||
					(network.file_path?.toLowerCase().includes(searchStr) ?? false)
				);
			}
		},
		{
